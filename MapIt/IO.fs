...
module Fs =
    ...
    [<Literal>]
    let MapItConfigName = "mapit.jsonc"

    type Paths() =
        static member GetMapItConfigPath(?path: string) =
            $"{defaultArg path (Environment.CurrentDirectory)}/{MapItConfigName}"

    let getMapitConfig filepath =
        try
            let bytes = File.ReadAllBytes filepath
            Json.FromBytes<MapitConfig> bytes |> Ok
        with
        | ex -> ex |> Error
    ...
    let createMapitConfig path config =
    ...
    let getorCreateLockFile configPath =
        taskResult {
            try
                ...
                return Json.FromBytes<MapItLock> bytes
            with
            | :? System.IO.FileNotFoundException -> return Map.ofList []
            | ex -> return! ex |> Error
        }

    let writeLockFile configPath (mapitLock: MapItLock) =
        let path = Path.GetFullPath($"%s{configPath}.lock")
        let serialized = Json.ToBytes mapitLock
